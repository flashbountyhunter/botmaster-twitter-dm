...
class TwitterDMBot extends BaseBot {
  ...
  __pollForMessages() {
	...
	this.twit.get('direct_messages/events/list',{count: 50}, function (err, data, response) {
		try {
			if (typeof data !== 'object' || typeof data.events !== 'object') return;
			
			let events = data.events;
			for (let i=0; i<events.length; i++){
				if (!self.lastEvent) self.lastEvent = events[0].id; // mark the newest event id as already processed (only process new events, not old existing events)
